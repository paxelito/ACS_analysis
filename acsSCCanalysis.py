	...
	if os.path.isdir(totDirName):
		...
		if os.path.isdir(resDirPath):
			...
			for ngen in range(1,numberOfGen+1):
				...
				print " - I N F L U X <--", influxFile
				# Influx in Protocell 
				if (influx_rate == 0) and (maxLOut > 0):
					speciesInFlux = range(0,(pow(2,(maxLOut+1)) - 2))
				else: # influx in CSTR
					try:
				...
				# Searching for files
				speciesFiles = os.path.join(tmpDir, tmpResFold)
				speciesFiles = speciesFiles + '/species_2_*'
				speciesFiles = sorted(glob.glob(speciesFiles))
				
				overThreshold = float(0)
				overThresholdTOT = float(0)
						
				for speciesFile in speciesFiles:
					...
					for sp in fidSpecies:
						...
						#if the species is not both in the influx and greater than the max length
					...
					if os.path.isdir(os.path.join(tmpDir, "res")):
						...
						for file in fileslist:
							# Create useful substrings starting from the file name
							tmpDirSplit = tmpDir.split("_")
							#print tmpDirSplit
							print tmpDirSplit
							filextPreInt = int(tmpDirSplit[tmpIDsim])
							print filextPreInt
							# e.g. "_[0*]nsim"
							filextPre = '_' + zeroBeforeStrNum(filextPreInt, len(tmpDirs)) + str(filextPreInt)
							print filextPre
							#open file
							fid = open(file, 'r')
							rctIDshow = 1.0
							...
							if checkIfGraphAlreadyExist() == 0:
								...
								for line in fid:
									...
									tmpReaction, tmpTime, tmpcc, tmpCat, tmpMol_I, tmpMol_II, tmpMol_III, tmpLoadedMols, tmpLoadedMolsConc, tmpGillMean, tmpGillSD, tmpGillEntropy = line.split()
									...
									if (rtime > rctIDshow * analysisTimeInterval):
										print "- ", tmpDir, " | Reaction ", reaction, " - time: ", rtime, " - Save structures"
										saveGraphToFile()
										saveGraphSUBToFile()
									...
									if ((rtime > rctIDshowNoSave * analysisTimeIntervalNoSave) & (printTemporalMessage == 1)):
										print "- ", tmpDir, " | Reaction ", reaction, " - time: ", rtime
								...
								if nrg == 1:
									...
									filename = '21_loadMols_CONC_' + str(rctParFileNum) + '.eps'
								...
								print "\t The reactions of this simulation have been already converted in graph format"
							...
							if len(IDsOverThreshold) > 0: # If there are species over threshols SOT
